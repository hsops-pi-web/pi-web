...
const PRESET_BUILTINS = new Set(PRESET_FULL);

export function getPresetFromTools(tools: ToolEntry[]): ToolPreset {
  const active = tools.filter(t => t.active && PRESET_BUILTINS.has(t.name)).map(t => t.name).sort().join(",");
  const anyActive = tools.some(t => t.active);
  if (active === "" && anyActive) return "default";
...
const PRESETS: { id: ToolPreset; label: string; desc: string; tools: string[] }[] = [
  ...
  { id: "default", label: "Low",  desc: "read · bash · edit · write · extensions", tools: PRESET_DEFAULT },
  { id: "full",    label: "High", desc: "read · bash · edit · write · grep · find · ls · extensions", tools: PRESET_FULL },
];
