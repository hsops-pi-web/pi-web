...
function gitReleaseSource(root: string): string {
  const source = join(root, "source");
  mkdirSync(join(source, "node_modules", "@earendil-works", "pi-coding-agent"), { recursive: true });
  writeFileSync(join(source, "package.json"), JSON.stringify({ version: "0.0.0-test" }));
  writeFileSync(join(source, "node_modules", "@earendil-works", "pi-coding-agent", "package.json"), JSON.stringify({ version: "0.0.0-test" }));
  writeFileSync(join(source, "tracked.txt"), "fixture\n");
  ...
  assert.equal(spawnSync("git", ["add", "."], { cwd: source }).status, 0);
  assert.equal(spawnSync("git", ["commit", "-m", "fixture"], { cwd: source }).status, 0);
  return source;
}
...
test("failed standalone verification never creates a release directory", () => {
  const root = tempRoot();
  const source = gitReleaseSource(root);
...
if grep -q '/opt/pi-web-auth/current' "$PI_WEB_TEST_INSTALLED_UNIT"; then mode=new; else mode=old; fi
...
`);
  ...
  const environment: NodeJS.ProcessEnv = {
    ...
    PI_WEB_UNIT_TEMPLATE: resolve("systemd/pi-web-auth.service.example"),
    PI_WEB_RELEASE_ENV: envFile,
...
test("standalone unit preserves production environment and stop guards", () => {
  const unit = readFileSync("systemd/pi-web-auth.service.example", "utf8");
  assert.match(unit, /WorkingDirectory=\/opt\/pi-web-auth\/current/);
  assert.match(unit, /EnvironmentFile=\/etc\/pi-web-auth\/release\.env/);
  assert.match(unit, /Environment=HOME=\/var\/lib\/pi-web-auth/);
  assert.match(unit, /Environment=PORT=8000/);
  assert.match(unit, /Environment=HOSTNAME=0\.0\.0\.0/);
  assert.match(unit, /ExecStart=.*node \/opt\/pi-web-auth\/current\/server\.js/);
