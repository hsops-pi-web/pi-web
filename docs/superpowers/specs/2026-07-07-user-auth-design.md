...
| 密码规则 | 长度 ≥ 8，且必须同时包含大写字母、小写字母、数字（不强制字母开头，不强制符号）。正则 `^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$`。注册页密码框侧边显示"至少 8 位，含大小写字母和数字"说明 |
...
3. 校验密码 `^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$`（长度 ≥ 8 且含大小写字母和数字），
   不符返 400。
...
- 密码框侧边说明"至少 8 位，含大小写字母和数字"，前端即时校验同一正则。
...
- 密码规则校验：合法（abcABC12、Passw0rd）；非法（长度<8、纯数字、纯字母、
  缺大写、缺小写、缺数字）均拒绝。
